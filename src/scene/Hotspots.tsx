...
function Hotspot({ spec, enabled }: { spec: HotspotSpec; enabled: boolean }) {
  ...
  /*
   * 每个热点都注册一个 useFrame：稳定态下 5 个热点 × 60fps = 300 次/秒
   * 进入 R3F 帧循环，但绝大多数热点既不在播 ripple 也不需要更新坐标。
   * 顶部这个快速分支把这两件事一并吃掉：先看 ripple 是否在播，不在就
   * 直接 return，连 performance.now() 都不读。anchor 同步路径单独处理，
   * 它只有在「物件移动」时才需要写位置。
   */
  useFrame(() => {
    const r = ripple.current
    const playing = rippleAt.current !== 0
    if (r && !r.visible && !playing) {
      // ripple 没在播、也不可见 —— 直接退出，连 performance.now() 都不读
      return
    }
