...
const SurfaceDraggable = forwardRef<Group, SurfaceDraggableProps>(function SurfaceDraggable(
  ...
  /*
   * 11+ 件物理物件原来都注册一份 useFrame，绝大部分时间 feedback 已经收敛到
   * 0、hovered 与 drag 也都是 false —— 仍然每帧进入 R3F 的 frame loop。
   * 改用手动 rAF：只在「可能让反馈变化」的事件里唤醒 tick，收敛到目标就退出。
   * R3F 的 frame loop 完全看不到这个组件，idle 状态零调度开销。
   *
   * 注意：这个 useCallback 必须在 endDrag / onPointerDown / onPointerOver /
   * onPointerOut / disabled 订阅 effect 等所有引用方之前定义，否则它们的
   * useCallback 依赖数组里读 startFeedback 时会撞上 TDZ。
   */
  ...
  const startFeedback = useCallback(() => {
    ...
    raf.current = requestAnimationFrame(tick)
  }, [enabled, hoverLift, hoverScale, invalidate, plane])
  ...
  const endDrag = useCallback(
    ...
    [enabled, invalidate, onTap, releaseCapture, rollback, startFeedback],
  )
  ...
  useEffect(() => {
    ...
    document.body.style.cursor = ''
    invalidate()
    // 让 feedback 从 1 收敛回 0，否则物件会在禁用状态下保持抬起的姿态
    startFeedback()
  }, [enabled, invalidate, onHoverChange, releaseCapture, rollback, startFeedback])
  ...
  /* startFeedback / 手动 rAF 的实现在文件前部,见 notifyPosition 之后 */
