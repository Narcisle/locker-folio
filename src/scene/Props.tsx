...
function IntroPhysical({ spec, children }: { spec: PropSpec; children: ReactNode }) {
  ...
  /*
   * 入场动画播完之后，整个 transform 已经被锁死在外层 SurfaceDraggable
   * 上，IntroPhysical 这层只负责初始那一段几百毫秒的过渡。R3F 的 useFrame
   * 不能自我注销，所以用一个 ref 把它降成"只跑一次"的形态：第一次 u 跨过 1
   * 时把 final transform 写进 group、置 done=true，之后每帧只读一个 bool。
   * 11 件物件 × 60fps = 660 次/秒的 setXxx 收掉。
   */
    ...
    <>
      ...
      >
        <PolaroidCardModel
          ...
          size={[0.34, 0.37]}
          photoUrl="/assets/photo/p11.webp"
        />
      </PhysicalItem>
    </>
  )
