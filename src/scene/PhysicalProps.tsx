...
export function PolaroidCardModel({
  ...
  /** 可选：直接用单张照片（风景照等），不再从 polaroids 图集裁剪 */
  photoUrl?: string
}) {
  const croppedTex = useCroppedTexture('/assets/obj2/polaroids.webp', POLAROID_SOURCE_SIZE, crop)
  const directTex = useTexture(photoUrl ?? '/assets/obj2/polaroids.webp')
  if (photoUrl) configurePaperTexture(directTex)
  const texture = photoUrl ? directTex : croppedTex
  const [cardWidth, cardHeight] = size
  const sourceAspect = photoUrl
    ? (directTex.image && (directTex.image as HTMLImageElement).width && (directTex.image as HTMLImageElement).height
        ? (directTex.image as HTMLImageElement).width / (directTex.image as HTMLImageElement).height
        : 1)
